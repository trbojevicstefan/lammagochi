...
export const CreatureCanvas3D = ({
  stage, stats, dayPhase, level, isStreaming,
  hatchProgress = 0, interactionSpark = 0,
  currentAnimation, skin = 'none',
}: Props) => {
  ...
  return (
    <div className={`viewport-container ${isEvolving ? 'viewport--shaking' : ''}`}>
      ...
      <div className="viewport-3d-layer">
        <Canvas camera={{ position: [0, 0, 4], fov: 50 }} gl={{ antialias: true, alpha: true }} dpr={[1, 1.5]}>
          ...
          {stage === 'hatching' ? (
            ...
            <>
              ...
              {interactionSpark > 0 && <ParticleEffects active type="sparkle" count={12} color="#fef3c7" />}
            </>
          )}
        </Canvas>
      </div>
      {stage === 'alive' && (
        <div className="viewport-pet-layer">
          <PixelPet level={level} mood={mood} dayPhase={dayPhase} isStreaming={isStreaming}
            interactionSpark={interactionSpark} actionAnimation={currentAnimation}
            emotion={emotionId} skin={skin as any} />
        </div>
      )}
      {stage === 'hatching' && (
        <div className="viewport-pet-layer">
